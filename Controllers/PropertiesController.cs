...
namespace CondoHub.Controllers
{
    ...
    public class PropertiesController : Controller
    {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            if (imovel == null)
                return NotFound();
            var hasContracts = await _context.Contracts.AnyAsync(c => c.PropertyId == id);
            if (hasContracts)
            {
                TempData["Error"] = "Não é possível excluir o imóvel enquanto existirem contratos vinculados. Remova ou reatribua-os primeiro.";
                return RedirectToAction(nameof(Detalhes), new { id });
            }
            ...
            {
                TempData["Error"] = "Erro ao excluir o imóvel. Verifique se há registros vinculados";
                return RedirectToAction(nameof(Detalhes), new { id });
            }
        }
    }
